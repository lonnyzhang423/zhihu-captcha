...
with tf.name_scope("input"):
    ...
    Y = tf.placeholder(tf.float32, [None, NUM_CLASSIFY_CLASSES], name="input_y")
...
def inference():
    ...
    with tf.variable_scope("final_output"):
        weights = tf.get_variable("weights",
                                  initializer=random_value(shape=[1024, NUM_CLASSIFY_CLASSES]))
        biases = tf.get_variable("biases",
                                 initializer=random_value(shape=[NUM_CLASSIFY_CLASSES], alpha=0.1))
...
def evaluation(logits, labels):
    with tf.variable_scope("accuracy"):
        predict = tf.reshape(logits, [-1, NUM_CLASSIFY_CLASSES])
        correct = tf.reshape(labels, [-1, NUM_CLASSIFY_CLASSES])

        max_idx_p = tf.argmax(predict, 1)
        max_idx_l = tf.argmax(correct, 1)
        ...
        try:
            for step in range(0, 10000):
                ...
                if step and step % 100 == 0:
                    file = os.path.join(check_points_dir(), "classify_model")
